...
======*/
resource "aws_ecr_repository" "shipit_ecr" {
  name = "${var.project_name}"
}
...
data "template_file" "web_task" {
  ...
  vars {
    image           = "${aws_ecr_repository.shipit_ecr.repository_url}:${var.project_version}"
    log_group       = "${aws_cloudwatch_log_group.log_group.name}"
  }
}

resource "aws_ecs_task_definition" "web" {
  family                   = "${var.project_name}_${var.environment}_web"
...
resource "aws_alb" "alb_shipit" {
  name            = "${var.environment}-alb-${var.project_name}"
...
resource "aws_alb_listener" "alb_listener_shipit" {
  load_balancer_arn = "${aws_alb.alb_shipit.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_ecs_service" "web" {
  ...
  /* If service is autoscaled up, don't reset */
  lifecycle {
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_${var.project_name}_web_cpu_utilization_high"
